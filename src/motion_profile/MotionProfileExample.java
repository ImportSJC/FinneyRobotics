...
public class MotionProfileExample {
	 ...
	 * keep one copy.
	 */
	private MotionProfileStatus[] _status = { new MotionProfileStatus(), new MotionProfileStatus() };
	...
	class PeriodicRunnable implements java.lang.Runnable {
	    public void run() {  
	    	_talon[RIGHT].processMotionProfileBuffer();    
	    	_talon[LEFT].processMotionProfileBuffer();    
	    	}
	}
	 ...
	 */
	public MotionProfileExample(MotorController left, MotorController right, double[][] leftMotionProfile, double[][] rightMotionProfile) {
		 ...
		 */
		_talon[RIGHT].changeMotionControlFramePeriod(5);
		_talon[LEFT].changeMotionControlFramePeriod(5);
	...
	public void control(int side) {
		/* Get the motion profile status every loop */
		_talon[side].getMotionProfileStatus(_status[side]);
		...
		if (_talon[side].getControlMode() != ControlMode.MotionProfile) {
			/*
			...
			/* Get the motion profile status every loop */
			_talon[side].getMotionProfileStatus(_status[side]);
			_heading = _talon[side].getActiveTrajectoryHeading();
			_pos = _talon[side].getActiveTrajectoryPosition();
	...
	private void startFilling(int side, double[][] profile, int totalCnt) {

		/* create an empty point */
		TrajectoryPoint point = new TrajectoryPoint();

		/* did we get an underrun condition since last time we checked ? */
		if (_status[side].hasUnderrun) {
			 ...
			 */
			_talon[side].clearMotionProfileHasUnderrun(0);
		}
		/*
		 * just in case we are interrupting another MP and there is still buffer
		 * points in memory, clear it.
		 */
		_talon[side].clearMotionProfileTrajectories();
